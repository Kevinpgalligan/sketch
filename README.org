...
- Bugfix: Use FBO for drawing with copy-pixels, prevents flickering on some systems. (It leads to visual artifacts on other systems, like the boundaries of triangles not being drawn correctly, so we probably need a flag to let the user pick how to handle drawing for copy-pixels).
