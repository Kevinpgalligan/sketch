...
- Bugfix: Polyline is janky for sharp angles (https://github.com/vydd/sketch/issues/53).
...
- Bugfix: Vertices are batched when drawing large shapes (https://github.com/vydd/sketch/issues/160).
...
Lines with an arbitrary number of segments with =polyline= (to adjust the line join algorithm, use the pen property =:line-join=):
...
***** Line join
=:line-join= is used to determine what sort of line join algorithm to use for joining line segments in a polyline. Valid options are =:dynamic=, =:bevel= and =:miter=. The =:dynamic= option tries to pick the best algorithm based on the angle between the line segments.

An example showing line join selection in action. Pressing spacebar changes the angle between line segments in the polyline; it can be seen that the miter join starts to malfunction when the angle is small.

#+BEGIN_SRC lisp
  (defsketch line-join-test ((deg 0))
    (translate (floor width 2) (floor height 2))
    (let ((coords (list -100 0 0 0 (* 150 (cos deg)) (* 150 (sin deg)))))
      (with-pen (:stroke +white+ :weight 5 :line-join :miter)
        (apply #'polyline coords)))
    (incf deg 0.1)
    (stop-loop))

  (defmethod on-key ((inst line-join-test) key state)
...
*** Setup
The generic function =(setup instance &key &allow-other-keys)= is a hook that gets called once on every "reset" of the sketch. That is:
...
*** Interactive development, errors and restarts
Sketch provides support for [[https://en.wikipedia.org/wiki/Interactive_programming][interactive development], allowing an entire sketch to be developed while it runs continuously in the background.

=(reset-sketch)= does what it says: resets the state of the sketch, as well as causing its setup method to be re-run.

=(reset-on :key-here)= specifies a keyboard shortcut to manually trigger the reset. By default, the keyboard shortcut is F1. Example:

#+BEGIN_SRC lisp
  (defsketch custom-reset-on
      ((reset-on :space)
	   (copy-pixels t))
    (circle (random width) (random height) 10))
#+END_SRC

When a running sketch is redefined (i.e. its defsketch form is recompiled), it gets reset. To prevent the reset, the option =(reset-on-change nil)= can be added to the sketch definition. To allow a particular parameter of the sketch to be tweaked, the =:tweakable t= flag can be added to it. In this example, a ball moves randomly around the screen. The color and radius of the ball can be tweaked by redefining the sketch, since both parameters have the =:tweakable= flag set; the position of the ball won't be affected.
...
When an error is encountered during the development of a sketch, Common Lisp's interactive debugger will be triggered and it'll show a list of restarts. After fixing whatever caused the error, the most important restarts to consider are =RESET-SKETCH= and =CONTINUE=. Both of these will begin the next iteration of the drawing loop (which, internally, is handled by SDL2), the difference being that =RESET-SKETCH= will first reset the sketch state. Alternatively, the debugger may provide an option to resume execution from a particular stack frame; in SLIME, press the "r" key over a particular frame to retry executing it.

If you just want to close the sketch window, then click the "x" in the window header (or press the ESC key) and select the =CONTINUE= restart. Otherwise, the debugger will continue to hog control of execution, and the drawing loop won't be able to process the exit event and close the window.
